...
async def main():
    ...
    # ── Save full CSV ──
    csv_path = 'backtest_details.csv'
    df.to_csv(csv_path, index=False)
    print(f"\nFull results saved to {csv_path}")

    # ── Detailed per-prediction printout (1H only, last 2 weeks) ──
    print("\n" + "=" * 100)
    print("DETAILED 1H PREDICTIONS (every hour)")
    print("=" * 100)
    print(f"{'Timestamp':<22} {'Pair':<8} {'Pred':<8} {'Conf':>6} {'Actual':<8} {'ActRet%':>8} {'OK?':>4}  "
          f"{'Q10':>7} {'Q25':>7} {'Q50':>7} {'Q75':>7} {'Q90':>7}  {'Entry':>9} {'Exit':>9}")
    print("-" * 130)

    df_1h = df[df['horizon'] == '1H'].sort_values(['timestamp', 'pair'])
    for _, r in df_1h.iterrows():
        mark = 'Y' if r['correct'] else 'N'
        print(f"{str(r['timestamp']):<22} {r['pair']:<8} {r['pred_dir']:<8} {r['confidence']:>6.3f} "
              f"{r['actual_dir']:<8} {r['actual_ret_pct']:>+8.4f} {mark:>4}  "
              f"{r['q10']:>7.4f} {r['q25']:>7.4f} {r['q50']:>7.4f} {r['q75']:>7.4f} {r['q90']:>7.4f}  "
              f"{r['entry_price']:>9.5f} {r['exit_price']:>9.5f}")

    # ── Summary: wrong predictions with high confidence ──
    print("\n" + "=" * 100)
    print("HIGH CONFIDENCE WRONG PREDICTIONS (>=65% conf, 1H)")
    print("=" * 100)
    wrong_hc = df_1h[(~df_1h['correct']) & (df_1h['confidence'] >= 0.65)].sort_values('confidence', ascending=False)
    print(f"Count: {len(wrong_hc)} out of {len(df_1h[df_1h['confidence'] >= 0.65])} high-conf predictions")
    print(f"{'Timestamp':<22} {'Pair':<8} {'Pred':<8} {'Conf':>6} {'Actual':<8} {'ActRet%':>8}  "
          f"{'Q10':>7} {'Q25':>7} {'Q50':>7} {'Q75':>7} {'Q90':>7}")
    print("-" * 110)
    for _, r in wrong_hc.iterrows():
        print(f"{str(r['timestamp']):<22} {r['pair']:<8} {r['pred_dir']:<8} {r['confidence']:>6.3f} "
              f"{r['actual_dir']:<8} {r['actual_ret_pct']:>+8.4f}  "
              f"{r['q10']:>7.4f} {r['q25']:>7.4f} {r['q50']:>7.4f} {r['q75']:>7.4f} {r['q90']:>7.4f}")
