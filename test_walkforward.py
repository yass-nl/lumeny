...
For each 1H timestamp T in the test window:
  1. Truncate all raw data to only bars closed by T
  2. Resample 4H/1D from truncated 1H (so partial bars are partial, like live)
  3. Compute features on truncated data
  4. Predict using models_2
  5. Check actual return from close[T] to close[T+1]

This replicates exactly what Railway/CandleBuffer sees at each hour.
"""
...
def build_features_at_time(raw_1h, raw_5m, raw_15m, pair, pair_idx,
    ...
    Truncates raw data, resamples 4H/1D from truncated 1H.
    Exactly what CandleBuffer.initialize() would produce.
    """
    # Truncate to bars fully closed by now_ts
    ...
    # Resample 4H/1D from truncated 1H — partial bars stay partial
    ...
    # Cross-pair correlations using truncated 1H closes
    closes_1h = {}
    for p in PAIRS:
        if p in all_raw_1h:
...
async def fetch_all_raw():
    """Fetch all raw data from Polygon once. We'll truncate per-timestamp later."""
    ...
    for pair in PAIRS:
        ...
        for tf_name, (multiplier, timespan, lookback_days) in tf_fetch_config.items():
            ...
            try:
                ...
                if not df.empty:
                    # Weekend filter (same as CandleBuffer)
...
async def main():
    ...
    # Get all 1H raw data dict for cross-pair correlations
    all_raw_1h = {}
    for pair in PAIRS:
        if '1H' in raw[pair]:
            all_raw_1h[pair] = raw[pair]['1H']

    # Get all unique 1H timestamps in test window (from any pair)
    all_timestamps = set()
    for pair in PAIRS:
        if '1H' in raw[pair]:
    ...
    for ts in all_timestamps:
        # now_ts = time when we'd run inference (5 min after bar close)
        ...
        for pair_idx, pair in enumerate(PAIRS):
            ...
            # Need actual return: close[T] to close[T+1]
            try:
            ...
            # Build features at this point in time
    ...
    # ── Report ──
    print("\n" + "=" * 80)
    print("WALK-FORWARD BACKTEST (NO LOOKAHEAD) — models_2, 1H, last 1 week")
    print("=" * 80)
    ...
    scored = df[df['confidence'] >= 0.60]
    print(f"\nScored predictions (>=60% conf): {len(scored)}")
    if len(scored) > 0:
        print(f"Accuracy: {scored['correct'].mean():.4f} ({scored['correct'].sum()}/{len(scored)})")
    ...
    for threshold in [0.55, 0.60, 0.65, 0.70, 0.80]:
        ...
        if len(hdf) == 0:
            ...
            print(f"  >= {threshold:.0%}     {acc:.4f}       {len(hdf):<8} {pct:.1f}%")
    ...
    for pair in PAIRS:
        ...
        if len(hc65) > 0:
            hc65_s = f"{hc65['correct'].mean():.3f} ({len(hc65)})"
        else:
            hc65_s = "N/A"

        hc70 = pdf[pdf['confidence'] >= 0.70]
        if len(hc70) > 0:
            hc70_s = f"{hc70['correct'].mean():.3f} ({len(hc70)})"
        else:
            hc70_s = "N/A"
    ...
    # Per-hour accuracy distribution
    by_ts = df.groupby('timestamp').agg(
        total=('correct', 'size'),
        correct=('correct', 'sum'),
    ).reset_index()
    by_ts['acc'] = by_ts['correct'] / by_ts['total']
    print("\n--- Per-Hour Accuracy Distribution ---")
    for n in range(8):
        count = len(by_ts[by_ts['correct'] == n])
        print(f"  {n}/7 correct: {count} hours")

    print("\n" + "=" * 80)
    print(f"Results saved to walkforward_results.csv")
    print("=" * 80)
